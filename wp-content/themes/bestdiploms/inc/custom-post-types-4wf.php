...
/**
 * Таксономия и пользовательский тип записи Другие документы (otherdoc).
 */
// 1. Регистрация новой таксономии (отсутствует).

// 2. Регистрация пользовательского типа записи Другие документы (otherdoc).
// At the top of functions.php you need to register your post type first.
add_action( 'init', function() {
    $type = 'otherdoc';
    $labels = xcompile_otherdoc_post_type_labels('Другой документ', 'Другие документы');
    
    // Declare what the post type supports
    $supports = ['title', 'editor', 'revisions', 'page-attributes', 'thumbnail', 'post-formats'/*, 'custom-fields'*/];

    $arguments = [
        'taxonomies' => ['post_tag', 'category', 'otherdoc'], // Add post tags and post categories
        'register_meta_box_cb' => 'otherdoc_meta_box', // Register a meta box
        //'register_meta_box_cb' => 'custom_meta_box_markup', // Register a meta box        
        'rewrite' => [ 'slug' => 'otherdocs' ], // Change the archive page URL
        'has_archive' => true, // Enable archive page
        'rest_base' => 'otherdoc',
        'show_in_rest' => true,
        'hierarchical' => true,
        'supports' => $supports,
        'public' => true,
        'description' => 'СPT Других документов сайта (например, Свидетельство о браке, Удостоверение интернатуры, Справка об обучении).',
...
// label our new post type as "Post" almost everywhere. We need the labels to be 
// primarily marked as "Study", not "Post".
function xcompile_otherdoc_post_type_labels($singular = 'Post', $plural = 'Posts') {
    //$p_lower = strtolower($plural);
    $p_lower = mb_convert_case($plural, MB_CASE_LOWER, "UTF-8");    
    //$s_lower = strtolower($singular);
    $s_lower = mb_convert_case($singular, MB_CASE_LOWER, "UTF-8");
...
// 3. Создание метабоксов для пользовательского типа записей Другие документы (otherdocs) в админпанели.
// Метабоксы:
// 1) Год выпуска
// 2) Бланк ГОЗНАК цена Р
// 3) Типографский цена Р
// The function for register_meta_box_cb
// Add form and basic text field
function otherdoc_meta_box(WP_Post $post) {
    add_meta_box('otherdoc_meta', 'Характеристики другого документа', function() use ($post) {
        $field_name1 = 'make';
        $field_name2 = 'gznk-price';
        $field_name = 'price';
        $field_value1 = get_post_meta($post->ID, $field_name1, true);
        $field_value2 = get_post_meta($post->ID, $field_name2, true);
        $field_value = get_post_meta($post->ID, $field_name, true);
        wp_nonce_field('otherdoc_nonce', 'otherdoc_nonce');
        ?>
...
// Check for empty string allowing for a value of `0` -> Working!
function empty_str_otherdoc( $str ) {
    return ! isset( $str ) || $str === "";
}
...
add_action('save_post', function($post_id){
    ...
    // Do not save meta for a revision or on autosave
    if ( $post->post_type != 'otherdoc' || $is_revision )
        return;
    ...
    // Secure with nonce field check
    if( ! check_admin_referer('otherdoc_nonce', 'otherdoc_nonce') )
        return;
    ...
    // Do the saving and deleting
    if( ! empty_str_otherdoc( $field_value1 ) ) {
        update_post_meta($post_id, $field_name1, $field_value1);
    } elseif( empty_str_otherdoc( $field_value1 ) ) {
        delete_post_meta($post_id, $field_name1);
    }

    if( ! empty_str_otherdoc( $field_value2 ) ) {
        update_post_meta($post_id, $field_name2, $field_value2);
    } elseif( empty_str_otherdoc( $field_value2 ) ) {
        delete_post_meta($post_id, $field_name2);
    }    
    
    if( ! empty_str_otherdoc( $field_value ) ) {
        update_post_meta($post_id, $field_name, $field_value);
    } elseif( empty_str_otherdoc( $field_value ) ) {
        delete_post_meta($post_id, $field_name);
    }    
} , 10, 3);

// 4. Регистрируем функцию подключения шаблона для вывода CPT 'otherdoc'. 
add_filter( 'template_include', 'include_otherdoc_function', 1 );
function include_otherdoc_function( $template_path ) {
    if ( get_post_type() == 'otherdoc' ) {
        if ( is_single() ) {
            // checks if the file exists in the theme first,
            // otherwise serve the file from the plugin
            if ( $theme_file = locate_template( array ( 'single-otherdoc.php' ) ) ) {
                $template_path = $theme_file; 
            } else {
                $template_path = plugin_dir_path( __FILE__ ) . '/single-otherdoc.php';
            }
        }
    }
    return $template_path;
